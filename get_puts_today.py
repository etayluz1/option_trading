...
4. Saves result to put_data_today/{ticker}.json
"""
...
def get_puts_for_ticker(stock_ticker, rules_file="rules_all.json"):
    ...
    # Create put_data_today directory if it doesn't exist
    os.makedirs("put_data_today", exist_ok=True)
    output_file = f"put_data_today/{stock_ticker}.json"
    ...
    # Save to put_data_today/{ticker}.json (directory already created at start)
    with open(output_file, 'w') as f:
        json.dump(result, f, indent=4)
