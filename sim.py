...
def _run_simulation_logic(rules_file_path, json_file_path):
    ...
    try:
        with open(rules_file_path, 'rb') as f:
            ...
            try:
                ...
                low_min_gain_to_take_profit_str = f"{LOW_MIN_GAIN_TO_TAKE_PROFIT*100:>13.4f}%" if LOW_MIN_GAIN_TO_TAKE_PROFIT is not None else f"{'N/A':>14}"
            except Exception:
    ...
    def compute_investable_flag(daily_data, min_5_day_rise_pct, min_above_avg_pct, max_above_avg_pct, min_avg_up_slope_pct, min_stock_price_rule):
        """Return True if the day's metrics satisfy the underlying_stock entry filters.

        This function operates on the already-loaded `daily_data` dict and uses
        the active rules passed in (which can be either standard or low mode rules).
...
# 5. Iterate chronologically over the unique dates, processing investable tickers
    ...
    for idx, date_str in enumerate(all_dates_list):
        ...
        if daily_date_obj >= start_date_obj:
            ...
            # Switch to low mode if positions drop to or below the minimum threshold
            if LOW_MIN_PUTS_TO_SET_LOW_MODE is not None and current_account_put_positions <= LOW_MIN_PUTS_TO_SET_LOW_MODE:
                if not low_puts_mode:
                    print(f"🔽 **MODE SWITCH: Entering LOW PUTs MODE** (Positions: {current_account_put_positions} <= {LOW_MIN_PUTS_TO_SET_LOW_MODE})")
                low_puts_mode = True
            
            # Switch to high mode if positions rise to or above the maximum threshold
            if LOW_MAX_PUTS_TO_SET_HIGH_MODE is not None and current_account_put_positions >= LOW_MAX_PUTS_TO_SET_HIGH_MODE:
                if low_puts_mode:
                    print(f"🔼 **MODE SWITCH: Entering HIGH PUTs MODE** (Positions: {current_account_put_positions} >= {LOW_MAX_PUTS_TO_SET_HIGH_MODE})")
                low_puts_mode = False
            
            # --- ACTIVE RULES SELECTION BASED ON MODE ---
            # Use low mode rules when in low_puts_mode, otherwise use standard rules
            active_max_puts_per_account = (LOW_MAX_PUTS_PER_ACCOUNT if low_puts_mode and LOW_MAX_PUTS_PER_ACCOUNT is not None 
                                          else MAX_PUTS_PER_ACCOUNT)
            active_max_puts_per_stock = (LOW_MAX_PUTS_PER_STOCK if low_puts_mode and LOW_MAX_PUTS_PER_STOCK is not None 
                                        else MAX_PUTS_PER_STOCK)
            active_max_puts_per_day = (LOW_MAX_PUTS_PER_DAY if low_puts_mode and LOW_MAX_PUTS_PER_DAY is not None 
                                      else MAX_PUTS_PER_DAY)
            
            # Convert percentage rules to comparable units (100x for percentages that are stored as decimals)
            active_min_5_day_rise_pct = (LOW_MIN_5_DAY_RISE_PCT * 100.0 if low_puts_mode and LOW_MIN_5_DAY_RISE_PCT is not None 
                                        else safe_percentage_to_float(rules['underlying_stock']['min_5_day_rise_pct']) * 100.0 if safe_percentage_to_float(rules['underlying_stock']['min_5_day_rise_pct']) is not None else None)
            active_min_above_avg_pct = (LOW_MIN_ABOVE_AVG_PCT * 100.0 if low_puts_mode and LOW_MIN_ABOVE_AVG_PCT is not None 
                                       else safe_percentage_to_float(rules['underlying_stock']['min_above_avg_pct']) * 100.0 if safe_percentage_to_float(rules['underlying_stock']['min_above_avg_pct']) is not None else None)
            active_max_above_avg_pct = (LOW_MAX_ABOVE_AVG_PCT * 100.0 if low_puts_mode and LOW_MAX_ABOVE_AVG_PCT is not None 
                                       else safe_percentage_to_float(rules['underlying_stock']['max_above_avg_pct']) * 100.0 if safe_percentage_to_float(rules['underlying_stock']['max_above_avg_pct']) is not None else None)
            active_min_avg_up_slope_pct = (LOW_MIN_AVG_UP_SLOPE_PCT * 100.0 if low_puts_mode and LOW_MIN_AVG_UP_SLOPE_PCT is not None 
                                          else safe_percentage_to_float(rules['underlying_stock']['min_avg_up_slope_pct']) * 100.0 if safe_percentage_to_float(rules['underlying_stock']['min_avg_up_slope_pct']) is not None else None)
            
            # Min stock price (direct value, not percentage)
            try:
                standard_min_stock_price = float(str(rules['underlying_stock'].get('min_stock_price', '')).replace('$', '').strip())
            except:
                standard_min_stock_price = None
            active_min_stock_price = (LOW_MIN_STOCK_PRICE if low_puts_mode and LOW_MIN_STOCK_PRICE is not None 
                                     else standard_min_stock_price)
            
            # Entry rules
            active_max_dte = (LOW_MAX_DAYS_FOR_EXPIRATION if low_puts_mode and LOW_MAX_DAYS_FOR_EXPIRATION is not None 
                             else MAX_DTE_RULE)
            active_min_bid_price = (LOW_MIN_PUT_BID_PRICE if low_puts_mode and LOW_MIN_PUT_BID_PRICE is not None 
                                   else MIN_BID_PRICE)
            active_min_delta = (LOW_MIN_PUT_DELTA if low_puts_mode and LOW_MIN_PUT_DELTA is not None 
                               else MIN_DELTA)
            active_max_delta = (LOW_MAX_PUT_DELTA if low_puts_mode and LOW_MAX_PUT_DELTA is not None 
                               else MAX_DELTA)
            active_max_spread_decimal = (LOW_MAX_ASK_ABOVE_BID_PCT if low_puts_mode and LOW_MAX_ASK_ABOVE_BID_PCT is not None 
                                        else MAX_SPREAD_DECIMAL)
            active_min_risk_reward_ratio = (LOW_MIN_RISK_REWARD_RATIO if low_puts_mode and LOW_MIN_RISK_REWARD_RATIO is not None 
                                           else MIN_RISK_REWARD_RATIO)
            active_min_annual_risk = (LOW_MIN_ANNUAL_RISK_REWARD_RATIO if low_puts_mode and LOW_MIN_ANNUAL_RISK_REWARD_RATIO is not None 
                                     else MIN_ANNUAL_RISK)
            active_min_rev_annual_risk = (LOW_MIN_REV_ANNUAL_RR_RATIO if low_puts_mode and LOW_MIN_REV_ANNUAL_RR_RATIO is not None 
                                         else MIN_REV_ANNUAL_RISK)
            active_min_expected_profit = (LOW_MIN_EXPECTED_PROFIT if low_puts_mode and LOW_MIN_EXPECTED_PROFIT is not None 
                                         else MIN_EXPECTED_PROFIT)
            
            # Exit rules
            active_position_stop_loss_pct = (LOW_POSITION_STOP_LOSS_PCT if low_puts_mode and LOW_POSITION_STOP_LOSS_PCT is not None 
                                            else POSITION_STOP_LOSS_PCT)
            active_stock_max_below_avg = (LOW_STOCK_MAX_BELOW_AVG if low_puts_mode and LOW_STOCK_MAX_BELOW_AVG is not None 
                                         else STOCK_MAX_BELOW_AVG_PCT)
            active_stock_max_below_entry = (LOW_STOCK_MAX_BELOW_ENTRY if low_puts_mode and LOW_STOCK_MAX_BELOW_ENTRY is not None 
                                           else STOCK_MAX_BELOW_ENTRY_PCT)
            active_take_profit_pct = (LOW_MIN_GAIN_TO_TAKE_PROFIT if low_puts_mode and LOW_MIN_GAIN_TO_TAKE_PROFIT is not None 
                                     else TAKE_PROFIT_MIN_GAIN_PCT)
            ...
            if active_stock_max_below_entry is not None and active_stock_max_below_entry > 0:
                ...
                for tr in open_trades_log:
                    ...
                    if entry_base is None or curr_px is None:
                        continue
                    threshold_px = entry_base * (1.0 - active_stock_max_below_entry)
            ...
            for i, trade in enumerate(open_trades_log):
                ...
                if current_adj_close is not None and sma150_adj_close is not None:
                    ...
                    threshold = sma150_adj_close * (1.0 - active_stock_max_below_avg)
                ...
                if current_bid_price is not None and entry_bid_price is not None and entry_bid_price > 0:
                    ...
                    if loss_ratio > 0 and loss_ratio >= active_position_stop_loss_pct:
                ...
                try:
                    if (
                        active_take_profit_pct is not None and active_take_profit_pct > 0 and
                        ...
                        if current_ask_price is not None and current_ask_price > 0:
                            ...
                            if profit_ratio >= active_take_profit_pct:
            ...
            if active_max_puts_per_account > 0:
                max_premium_per_trade_today = total_account_value / float(active_max_puts_per_account)
            else:
            ...
            print(f"📈 Max Premium per Trade (today, NAV/{active_max_puts_per_account}): ${max_premium_per_trade_today:,.2f}")
            ...
            if current_account_put_positions >= active_max_puts_per_account and not account_full_today:               
                print(f"🛑 **ACCOUNT FULL (Global Limit):** {current_account_put_positions}/{active_max_puts_per_account} contracts. Skipping scan for new trades.")
            ...
            if not account_full_today:
                ...
                for ticker in all_tickers:
                    ...
                    if date_str in stock_history_dict[ticker]:
                        # Compute investable dynamically from the day's metrics and active rules
                        daily_data = stock_history_dict[ticker][date_str]
                        is_investable = compute_investable_flag(daily_data, active_min_5_day_rise_pct, active_min_above_avg_pct, 
                                                                active_max_above_avg_pct, active_min_avg_up_slope_pct, active_min_stock_price)
                        ...
                        if open_puts_tracker[ticker] >= active_max_puts_per_stock:
                            continue
                        ...
                        if daily_orats_data and ticker in daily_orats_data:
                            ...
                            for expiration_date, exp_data in ticker_orats_data.items():
                                ...
                                if isinstance(days_interval, int) or (isinstance(days_interval, str) and str(days_interval).isdigit()):
                                    ...
                                    if MIN_DTE_RULE <= dte <= active_max_dte:
                                        ...
                                        for option in options_array:
                                            ...
                                            passes_bid = pbidpx_value > active_min_bid_price
                                            if not passes_bid: continue # Fail fast

                                            # --- Check 2: Put Delta ---                                            
                                            passes_delta = active_min_delta <= put_delta_value <= active_max_delta
                                            if not passes_delta: continue # Fail fast

                                            # --- Check 3: Bid-Ask Spread ---
                                            passes_spread = pbidpx_value > 0 and paskpx_value > pbidpx_value and ((paskpx_value - pbidpx_value) / pbidpx_value) <= active_max_spread_decimal
                                            ...
                                            if pbidpx_value > 0 and strike_value > pbidpx_value:
                                                ...
                                                try:
                                                    ...
                                                    expected_profit = None
                                                passes_rr = risk_reward_ratio is not None and risk_reward_ratio > active_min_risk_reward_ratio
                                                passes_annual = annual_risk is not None and annual_risk > active_min_annual_risk
                                                passes_rev_annual = annual_risk_reverse is not None and annual_risk_reverse > active_min_rev_annual_risk
                                                passes_expected = expected_profit is not None and expected_profit > active_min_expected_profit
                ...
                # --- DAILY TRADE ENTRY LOOP (up to active_max_puts_per_day new positions) ---
                while daily_entries_count < active_max_puts_per_day:
                    ...
                    if daily_trade_candidates:
                        ...
                        for contract in daily_trade_candidates:
                            ...
                            if open_puts_tracker[ticker_check] >= active_max_puts_per_stock:
                                continue 
                            ...
                            if unique_key not in active_position_keys:
                                ...
                                remaining_account_position_slots = active_max_puts_per_account - current_account_put_positions
                                remaining_stock_position_slots = active_max_puts_per_stock - open_puts_tracker[ticker_check]
