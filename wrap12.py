...
wrapper_sweep_pct_set = [28, 20, 5, 36, 45, 8, 2, 32]  # Percentages

wrap_group_size = 2  # Group size for optimization sweeps (3 or 4)
...
def main(wrapper_sweep_pct_group: list[float], wrap_id_offset: int = 0) -> None:
    ...
    try:
        ...
        for rule_id, spec in enumerate(param_specs, start=1):
            ...
            assigned_values = set()
            for wrap_id, sweep_pct in enumerate(wrapper_sweep_pct_group, start=1 + wrap_id_offset):
...
if __name__ == "__main__":
    ...
    wrap_id_offset = 0  # Track wrap_id offset across groups
    for group_idx, group in enumerate(groups, start=1):
        print()
        print(f"Starting optimization group {group_idx}/{len(groups)}: sweep percentages = {group}")
        print()
        main(group, wrap_id_offset)
        wrap_id_offset += len(group)  # Increment offset by the number of sweep percentages in this group
